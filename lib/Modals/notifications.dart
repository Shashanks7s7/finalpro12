...
class Noti {
  ...
  Future<void> notifications() async {
    FlutterLocalNotificationsPlugin flutterp =
        FlutterLocalNotificationsPlugin();
    var va = '2021-07-19 12:4:30';
    print(DateTime.now().toLocal());
    var date = DateTime.now(); //DateTime.parse(va);
    List<dynamic> scheduledDate = [
      date.add(const Duration(seconds:20)),
      date.add(const Duration(seconds: 50)),
      date.add(const Duration(seconds: 150))
    ];
    ...
    //       tz.local), //tz.TZDateTime.now(tz.local).add(const Duration(seconds: 5)),
